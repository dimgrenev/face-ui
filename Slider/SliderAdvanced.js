...
export function SliderAdvanced({ rootRef, value, defaultValue, scalarValue, min = 0, max = 100, step = 1, disabled = false, label, className, leading = 'none', leadingIcon, leadingText, crop = false, defaultCropRange, cropRange, cropLocksValue = true, onChange, onValueChange, onCropChange, }) {
    var _a;
    ...
    const controlledValue = resolveScalar(scalarValue !== null && scalarValue !== void 0 ? scalarValue : value);
    const defaultScalar = (_a = resolveScalar(defaultValue)) !== null && _a !== void 0 ? _a : mn;
    ...
    const normalizeCrop = (next) => {
        const a = snapToStep(Number(next === null || next === void 0 ? void 0 : next.min));
        const b = snapToStep(Number(next === null || next === void 0 ? void 0 : next.max));
        let lo = Math.min(a, b);
        let hi = Math.max(a, b);
        const minGap = safeStep;
        if ((mx - mn) >= minGap && (hi - lo) < minGap)
            hi = clamp(lo + minGap, mn, mx);
        lo = clamp(lo, mn, mx);
        hi = clamp(hi, mn, mx);
        if (hi < lo)
            hi = lo;
    ...
    const [internalValue, setInternalValue] = useState(() => snapToStep(isControlled ? controlledValue : defaultScalar));
    const [internalCrop, setInternalCrop] = useState(() => {
        const initialCrop = defaultCropRange !== null && defaultCropRange !== void 0 ? defaultCropRange : { min: mn, max: mx };
    ...
    const thumbMeasureRef = useRef(null);
    const draggingRef = useRef(false);
    const dragKindRef = useRef(null);
    const dragOffsetRef = useRef(0);
    useEffect(() => {
        if (!isControlled || draggingRef.current)
            return;
        setInternalValue(snapToStep(controlledValue));
    ...
    useEffect(() => {
        if (!cropEnabled)
            return;
        const next = normalizeCrop(isCropControlled ? cropRange : internalCrop);
        if (!isCropControlled) {
            setInternalCrop((prev) => (prev.min === next.min && prev.max === next.max ? prev : next));
        }
    }, [cropEnabled, isCropControlled, cropRange === null || cropRange === void 0 ? void 0 : cropRange.min, cropRange === null || cropRange === void 0 ? void 0 : cropRange.max, internalCrop, mn, mx, safeStep]);
    const committedCrop = useMemo(() => {
        if (!cropEnabled)
            return { min: mn, max: mx };
        return normalizeCrop(isCropControlled ? cropRange : internalCrop);
    ...
    const normalizeValueToCrop = (next, cropNow) => {
    ...
    useEffect(() => {
        if (!cropLocks || draggingRef.current)
            return;
        const normalized = normalizeValueToCrop(committedValue, committedCrop);
        if (!Number.isFinite(normalized) || normalized === committedValue)
            return;
        if (!isControlled)
            setInternalValue(normalized);
        onChange === null || onChange === void 0 ? void 0 : onChange(normalized);
        onValueChange === null || onValueChange === void 0 ? void 0 : onValueChange({ value: [normalized] });
    ...
    const decimals = useMemo(() => {
        ...
        return match ? Math.min(6, Math.max(0, (_b = (_a = match[1]) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0)) : 0;
    }, [safeStep]);
    ...
    const tFromValue = (next) => {
        const range = mx - mn;
        if (!Number.isFinite(next) || !Number.isFinite(range) || range <= 0)
            return 0;
    ...
    const emitCrop = (next) => {
        ...
        onCropChange === null || onCropChange === void 0 ? void 0 : onCropChange(normalized);
    };
    const startThumbDrag = (clientX, rect) => {
        ...
        const onMove = (nextEvent) => {
            if (!draggingRef.current || dragKindRef.current !== 'thumb' || !geom.ok)
                return;
    ...
    const startCropDrag = (which, clientX, rect) => {
        draggingRef.current = true;
        ...
        const onMove = (nextEvent) => {
            if (!draggingRef.current || !geom.ok)
                return;
            const x = (nextEvent.clientX - rect.left) - EDGE_GAP_PX;
            const desiredLeft = x - dragOffsetRef.current;
            ...
            if (which === 'min')
                markerLeft = Math.min(markerLeft, otherLeft - minSpanPx);
            else
                markerLeft = Math.max(markerLeft, otherLeft + minSpanPx);
            if (cropLocks) {
                if (which === 'min')
                    markerLeft = Math.min(markerLeft, (thumbLeft - BUFFER_PX) - MARKER_W_PX);
                else
                    markerLeft = Math.max(markerLeft, thumbRight + BUFFER_PX);
            }
            ...
            const nextCrop = normalizeCrop(which === 'min'
                ? { min: nextValue, max: (dragCrop !== null && dragCrop !== void 0 ? dragCrop : committedCrop).max }
                : { min: (dragCrop !== null && dragCrop !== void 0 ? dragCrop : committedCrop).min, max: nextValue });
    ...
    const handlePointerDown = (event) => {
        ...
        if (disabled)
            return;
        const rect = (_a = bodyRef.current) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect();
        if (!rect || rect.width <= 0)
            return;
        event.preventDefault();
        event.stopPropagation();
        (_c = (_b = inputRef.current) === null || _b === void 0 ? void 0 : _b.focus) === null || _c === void 0 ? void 0 : _c.call(_b);
        try {
            const target = event.target;
            if (cropEnabled && ((_d = target === null || target === void 0 ? void 0 : target.closest) === null || _d === void 0 ? void 0 : _d.call(target, '.uf-slider-cropMarker--left'))) {
                startCropDrag('min', event.clientX, rect);
                return;
            }
            if (cropEnabled && ((_e = target === null || target === void 0 ? void 0 : target.closest) === null || _e === void 0 ? void 0 : _e.call(target, '.uf-slider-cropMarker--right'))) {
    ...
    const sliderClasses = cn('uf-slider', 'uf-slider--advanced', disabled && 'uf-slider--disabled', cropEnabled && 'uf-slider--crop', className);
    const resolvedLeadingIcon = typeof leadingIcon === 'string'
        ? _jsx(Icon, { name: leadingIcon })
        : leadingIcon;
    const leadingNode = (() => {
        if (leading === 'none')
            return null;
        const iconNode = resolvedLeadingIcon ? _jsx("span", { className: "uf-slider-leadingIcon", children: resolvedLeadingIcon }) : null;
        const textNode = leadingText ? (_jsx(Text, { membrane: false, variant: "body", className: "uf-slider-leadingText", children: leadingText })) : null;
        if (leading === 'icon')
            return iconNode;
        if (leading === 'text')
            return textNode;
